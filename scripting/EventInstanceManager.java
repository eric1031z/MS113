...
public class EventInstanceManager {
    ...
    public void restartEventTimer(long time) {
        if (ChannelServer.getInstance(1).isShutdown()) {
            System.err.println("restartEventTimer - 關閉伺服器中無法調用");
            return;
        }
        try {
            if (disposed) {
    ...
    public final void saveBossQuest(final int points) {
        ...
        for (MapleCharacter chr : getPlayers()) {
            ...
            if (record.getCustomData() != null) {
                record.setCustomData(String.valueOf(points + Long.parseLong(record.getCustomData())));
            } else {
    ...
    public void dispose_NoLock() {
        if (ChannelServer.getInstance(1).isShutdown()) {
            System.err.println("dispose_NoLock - 關閉伺服器中無法調用");
            return;
        }
        if (disposed || em == null) {
            return;
    ...
    public void registerSquad(MapleSquad squad, MapleMap map, int questID) {
        registerSquad(squad, map, questID, null);
    }

    public void registerSquad(MapleSquad squad, MapleMap map, int questID, String bossname) {
        if (disposed) {
            return;
        }

...
import static tools.packet.MobPacket.addMonsterStatus;
...
public class MaplePacketCreator {
    ...
    /*public static final byte[] setNPCScriptable(int npcId, String description) {
...
//        mplew.write(summon.getSkill() == 32111006 ? 5 : 4); //reaper = 5?
    ...
    public static byte[] getMegaphone(String message) {
        return broadcastMessage(2, 0, new String[]{message}, false, null);
    }

    public static byte[] getSuperMegaphone(String message, boolean whisper, int channel) {
    ...
    public static byte[] itemMegaphone(String message, boolean whisper, int channel, IItem item) {
        return broadcastMessage(8, channel, new String[]{message}, whisper, item);
    }

    public static byte[] getTripleSmega(List<String> message, boolean ear, int channel) {
        return broadcastMessage(10, channel, (String[]) message.toArray(new String[message.size()]), ear, null);
    }

    public static byte[] getHeartSmega(String message, boolean ear, int channel) {
        return broadcastMessage(11, channel, new String[]{message}, ear, null);
    }

    public static byte[] getSkullSmega(String message, boolean ear, int channel) {
        return broadcastMessage(12, channel, new String[]{message}, ear, null);
    }

    public static byte[] getGachaponMega(final String message, final IItem item, int channel) {
        return broadcastMessage(13, channel, new String[]{message}, false, item);
    }

    public static byte[] broadcastMessage(int type, String message) {
...
//        mplew.write(0);
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        if (chr.getBuffedValue(MapleBuffStat.BERSERK_FURY) != null) {
            buffmask |= 0x2000000;//MapleBuffStat.BERSERK_FURY.getOldValue();
        }
        ...
        if (buffSrc > 0) {
            ...
            if (GameConstants.getMountItem(buffSrc) == 0 && c_mount != null) {
                ...
                if(buffSrc == 1004){
                    //mplew.writeInt(chr.getCurrentMount(chr.getId()));
                    mplew.writeInt(chr.getchairTam(chr.getChair()));
                }else{
...
//        mplew.write(isSelf ? 1 : 0);
    ...
    public static byte[] giveEnergyCharge(int bar, int bufflength) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] giveEnergyCharge(int cid, int bar, int bufflength) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.writeShort(c.getPlayer().getJob());
    ...
    public static byte[] partyInvite(MapleCharacter from, boolean auto) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.writeInt(skillId);
    ...
    public static byte[] requestBuddylistAdd(int cidFrom, String nameFrom) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        mplew.write(1);
        mplew.writeInt(0);
        mplew.writeAsciiString("尚未設定群組", 17);
        mplew.writeShort(0);
...
//        mplew.write(0); //?
        ...
        switch (operation) {
            ...
            case 10: { // Open duey
                ...
                for (MapleDueyActions dp : packages) {
                    ...
                    mplew.writeShort(0);
                    mplew.writeAsciiString(dp.getMsg(), 193); //訊息
                    mplew.writeZeroBytes(10);
                    if (dp.getItem() != null) {
                        mplew.write(1);
    ...
    public static class AntiMacro {
        ...
        public static byte[] getImage(byte action, File file, int times) {
            return antiMacroResult((byte) 6, action, null, file, times);
        }
    ...
    public static byte[] getOwlOpen() { //best items! hardcoded
        ...
        mplew.write(7);
        mplew.write(ServerConstants.owlItems.length);
        for (int i : ServerConstants.owlItems) {
            mplew.writeInt(i);
    ...
    public static byte[] getOwlSearched(final int itemSearch, final List<HiredMerchant> hms) {
        ...
        for (Map.Entry<MaplePlayerShopItem, HiredMerchant> entry : resultItems.entrySet()) {
            ...
            mplew.write(entry.getValue().getMap().getChannel() - 1);// 頻道
    ...
    public static byte[] DamageSkin(int cid, int skinID){
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(0x0570);
        mplew.writeInt(cid);

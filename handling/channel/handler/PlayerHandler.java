...
import static constants.GameConstants.isMonsterRiding;
...
public class PlayerHandler {
    ...
    public static void ThrowGrenade(final LittleEndianAccessor slea, final MapleCharacter chr) {
        // 沒處理
    }
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse == null && itemId >= 3010000 && itemId < 9999999) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("[封鎖系統] " + c.getPlayer().getName() + " 因為修改封包而被管理員永久停權。"));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") 修改椅子(" + itemId + ")封包，坐上椅子時封鎖。 身上並沒有該物品"));
...
//        }
        ...
        chr.setCurrentMount(chr.getId(), chr.getchairTam(chr.getChair()));
        SkillFactory.getSkill(1013).getEffect(1).applyTo(chr);
        c.sendPacket(MaplePacketCreator.enableActions());
    }
    ...
    public static final void TakeDamage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (MapConstants.isCakeMap(chr.getMapId())) {
...
//            double distance = attacker.getPosition().distance(chr.getPosition());
//            if (attacker.getPosition().distance(chr.getPosition()) > 150) {
//                if (damage > 0 && !attacker.getStats().isBoss() && !chr.checkUsingMoveskillTime()) {
//                    chr.getCheatTracker().registerOffense(CheatingOffense.MOUSE_MOVED_CS, " [CS滑鼠移動] 頻道:" + chr.getClient().getChannel() + " 地圖:" + chr.getMapId() + " 範圍: " + (int) distance);
//                    //chr.dropMessage("[CS滑鼠移動] 觸發: " + true + " 範圍: " + (int) distance);
...
//            }
    ...
    public static final void UseItemEffect(final int itemId, final MapleClient c, final MapleCharacter chr) {
        final MapleInventoryType type = GameConstants.getInventoryType(itemId);
        IItem toUse = null;
        if (chr != null && chr.getInventory(type) != null) {
...
//            return;
        ...
        if (skillid == 狂狼勇士1.戰鬥衝刺) {
            c.getPlayer().updateUsingMoveSkill_sp(2000, true);
        } else if (SkillConstants.is瞬間移動(skillid)) {
            c.getPlayer().updateUsingMoveSkill_sp(500, true);
        } else if (SkillConstants.is二段跳(skillid)) {
            c.getPlayer().updateUsingMoveSkill_sp(1500, true);
        } else if (SkillConstants.isMoveSkill(skillid)) {
            c.getPlayer().updateUsingMoveSkill(true);
        }
        ...
        if (effect.getCooldown() > 0) {
            ...
            if (skillid != 槍神.海盜船) {
                if (chr.isAdmin() && chr.isInvincible()) {
                    chr.dropMessage("伺服器管理員無敵狀態消除技能冷卻[" + SkillFactory.getName(skillid) + "(" + skillid + ")]，持續時間：" + effect.getCooldown() + "秒");
                } else {
...
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, slea.readByte()), chr.getPosition());
    ...
    public static final void closeRangeAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        if (attack.skill != 0) {
            ...
            if (effect.getCooldown() > 0) {
                ...
                if (chr.isAdmin() && chr.isInvincible()) {
                    chr.dropMessage("伺服器管理員無敵狀態消除技能冷卻[" + SkillFactory.getName(attack.skill) + "(" + attack.skill + ")]，持續時間：" + effect.getCooldown() + "秒");
                } else {
        ...
        chr.checkFollow();
        chr.getMap().broadcastMessage(chr, MaplePacketCreator.DamageSkin(chr.getId(), chr.getDamageSkin()),chr.getPosition()); //先呼叫傷害    
    ...
    public static final void rangedAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (attack.skill != 0) {
            ...
            if (effect.getCooldown() > 0) {
                ...
                if (SkillConstants.isMoveSkill(attack.skill)) {
                    c.getPlayer().updateUsingMoveSkill(true);
                }
                if (chr.isAdmin() && chr.isInvincible()) {
                    chr.dropMessage("伺服器管理員無敵狀態消除技能冷卻[" + SkillFactory.getName(attack.skill) + "(" + attack.skill + ")]，持續時間：" + effect.getCooldown() + "秒");
        ...
        if (attack.AOE != 0 && chr.getBuffedValue(MapleBuffStat.SOULARROW) == null && attack.skill != 4111004) {
            if (chr.getInventory(MapleInventoryType.USE).getItem(attack.slot) == null && attack.skill != 3121004) {
                //chr.getCheatTracker().registerOffense(CheatingOffense.無箭矢發射弓箭, " 技能: " + attack.skill + " 目標數量: " + attack.targets);
                return;
            }
    ...
    public static final void Heal(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (healHP != 0 && chr.canHP()) {
            int check = (int) stats.getShouldHealHP();
            if (healHP > check * 5) {
                //chr.getCheatTracker().registerOffense(CheatingOffense.REGEN_HIGH_HP, String.valueOf(healHP) + "服務端:" + check);
            }
            chr.addHP(healHP);
        }

        if (healMP != 0 && chr.canMP()) {
            int check = (int) stats.getShouldHealMP();
            if (healMP > check * 5) {
                //  chr.getCheatTracker().registerOffense(CheatingOffense.REGEN_HIGH_MP, String.valueOf(healMP) + "服務端:" + check);
            }
            chr.addMP(healMP);
        }
...
//	slea.skip(5); // unknown
        ...
        if (res != null && c.getPlayer().getMap() != null) { // TODO more validation of input data
            if (slea.available() < 13 || slea.available() > 26) {
                System.err.println("slea.available != 13-26 (movement parsing error)\n" + slea.toString(true));
                return;
            }
...
//	    }
            ...
            CheckMove(chr, Original_Pos, pos);
...
//            slea.skip(6); //D3 75 00 00 00 00
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (!wheel) {
                    ...
                    chr.dropMessage(-5, "原地復活術數量剩下" + chr.getInventory(MapleInventoryType.CASH).countById(5510000) + "個。");
...
//	slea.readShort(); // Original Y pos
    ...
    public static void CheckMove(MapleCharacter chr, Point oldpos, Point newpos) {
        if (!chr.checkUsingMoveskillTime()) {
            double oldx = oldpos.getX(), newx = newpos.getX();
            double oldy = oldpos.getY(), newy = newpos.getY();
            double distance = Math.abs(newx - oldx);
            double distance_y = Math.abs(newy - oldy);
            double correct = 190, correct_y = 380;
            if (!chr.checkUsingPortal()) {
                if (!chr.checkWarpingMap() || distance > 2000) {
                    if (distance > correct || distance_y > correct_y) {
                        chr.addMove();
                        if (chr.isStaff()) {
                            chr.dropMessage(5, "[觸發] x: " + distance + " y:" + distance_y + " 次數: " + chr.getAddMove() + " 判斷: " + correct + ", " + correct_y);
                        }
                    } else {
                        if (chr.getWalkDebugMessage()) {
                            chr.dropMessage(5, "[未觸發] x: " + distance + " y:" + distance_y + " 次數: " + chr.getAddMove() + " 判斷: " + correct + ", " + correct_y);
                        }
                    }
                    if (chr.getAddMove() > 4) {
                        if (chr.isPlayer()) {
                            //chr.getClient().getSession().close();
                            //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("玩家[" + chr.getName() + "] 地圖[" + chr.getMapId() + "] 原始座標[" + oldpos.getX() + "," + oldpos.getY() + "] 新座標[" + newpos.getX() + "," + newpos.getY() + "] 相差x:" + distance + " 相差y:" + distance_y + " 服務端預計: " + correct));
                        } else {
                            chr.dropMessage(5, "觸發自動斷線");
                        }
                        //FileoutputUtil.logToFile("Logs/Hack/移動異常/" + chr.getName() + ".txt", FileoutputUtil.CurrentReadable_Time() + " 玩家[" + chr.getName() + "] 地圖[" + chr.getMapId() + "] 原始座標[" + oldpos.getX() + "," + oldpos.getY() + "] 新座標[" + newpos.getX() + "," + newpos.getY() + "] 相差x:" + distance + " 相差y:" + distance_y + " 服務端預計: " + correct);
                        chr.clearAddMove();
                    }
                }
            }

    ...
    public final MapleStatEffect getAttackEffect(final MapleCharacter chr, int skillLevel, final ISkill skill_) {
        ...
        if (GameConstants.isLinkedSkill(skill)) {
            ...
            if (skill != skill_.getId()) {
                FileoutputUtil.logToFile("logs/data/AttackEffect.txt", "" + FileoutputUtil.NowTime() + " 連結技能[" + skill + "](" + skill_.getId() + "傳承) 連結技能等級:" + skillLevel + " 不在getLinkedkill清單內卻被觸發, 觸發者: " + chr.getName() + " 職業: " + chr.getJob() + " 等級: " + chr.getLevel() + "\r\n");
            }
        }
        if (display > 80) {
            if (!skill_.hasAction()) {

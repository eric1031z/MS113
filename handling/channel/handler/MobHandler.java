...
public class MobHandler {
    ...
    public static final void MoveMonster(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        try {
            ...
            //  World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語] " + chr.getName() + " (編號: " + chr.getId() + ") 怪物移動錯誤(" + monster.getId() + ")! - 地圖:" + chr.getMapId()), true);
            //   FileoutputUtil.log(FileoutputUtil.Movement_Log, "怪物移動錯誤 AIOBE Type2 : 玩家: " + c.getPlayer().getName() + "(編號" + c.getPlayer().getId() + ") 怪物ID " + monster.getId() + "\r\n錯誤訊息:" + e + "\r\n封包:\r\n" + slea.toString(true));
            return;
        }
    ...
    public static final void handleAutoAggro(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (monster != null && chr.getPosition().distanceSq(monster.getPosition()) < 200000) { //為了不讓那低能怪物自己找仇恨
            ...
            if (!fly) {
                ...
                if (((reduce_x > GeneallyDistance_x || reduce_y > GeneallyDistance_y) && reduce_y != 0) || (reduce_x > Check_x && reduce_y == 0) || reduce_x > max_x) {
                    ...
                    if (c.getPlayer().getMobVac() % 50 == 0 || reduce_x > max_x) {
                        ...
                        //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語] " + chr.getName() + " (編號: " + chr.getId() + ")使用吸怪(" + chr.getMobVac() + ")! - 地圖:" + chr.getMapId() + "(" + chr.getMap().getMapName() + ")"), true);
            ...
            if (!fly) {
                ...
                if (reduce_x > GeneallyDistance_x) {
                    ...
                    if (c.getPlayer().getMobVac_X() % 10 == 0) {
                        //c.getPlayer().getCheatTracker().registerOffense(CheatingOffense.MOB_VAC_X, "(地圖: " + chr.getMapId() + " 怪物數量:" + chr.getMobVac_X() + ")");
                        //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語] " + chr.getName() + " (編號: " + chr.getId() + ")使用X座標吸怪(" + chr.getMobVac_X() + ")! - 地圖:" + chr.getMapId() + "(" + chr.getMap().getMapName() + ")"), true);
                        StringBuilder sb = new StringBuilder();
                        sb.append("\r\n");
                        sb.append(FileoutputUtil.NowTime());
                        sb.append(" 玩家: ");
                        ...
                        if (chr.hasGmLevel(1)) {
                            //c.getPlayer().dropMessage("觸發吸怪 --  x: " + reduce_x);
                        }
                    }
                }
            }

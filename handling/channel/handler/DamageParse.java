    ...
    public static void applyAttack(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
        ...
        if (attack.skill != 0) {
            ...
            if (GameConstants.isPyramidSkill(attack.skill)) {
                if (player.getMapId() / 1000000 != 926 || player.getPyramidSubway() == null || !player.getPyramidSubway().onSkillUse(player)) {
            ...
            if (GameConstants.isAran(player.getJob())) {
                int reduce = player.CountAranCombo(attack.skill);
                if (reduce > 0) {
                    if (player.getCombo() - reduce < 0) {
                        //player.getCheatTracker().registerOffense(CheatingOffense.ARAN_COMBO_HACK, "COMBO數量" + player.getCombo() + "使用需要" + reduce + " COMBO的技能(" + attack.skill + ")");
                    }
            ...
            if (attack.hits > last*500) {
                if (player.hasGmLevel(1)) {
                    ...
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM 密語系統] " + player.getName() + " (等級 " + player.getLevel() + ") 攻擊次數異常已經紀錄。 玩家攻擊次數 " + attack.hits + " 正確次數 " + last + " 技能ID " + attack.skill));
            ...
            if (attack.targets > CheckCount*500) {
                if (player.hasGmLevel(1)) {
                    player.dropMessage("打怪數量異常,技能代碼: " + attack.skill + " 封包怪物量 : " + attack.targets + " 服務端怪物量 :" + CheckCount);
                } else {
                    //player.getCheatTracker().registerOffense(CheatingOffense.攻擊怪物數量異常, " 玩家: " + player.getName() + "(" + player.getLevel() + ") 地圖: " + player.getMapId() + "技能代碼: " + attack.skill + " 技能等級: " + player.getSkillLevel(effect.getSourceId()) + " 封包怪物量 : " + attack.targets + " 服務端怪物量 :" + CheckCount);
                    ...
                    //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM 密語系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "攻擊怪物數量異常。 " + "封包怪物量 " + attack.targets + " 服務端怪物量 " + effect.getMobCount() + " 技能ID " + attack.skill));
                    return;
                }
            }
        }
        ...
        if (attack.skill == SkillType.神偷.落葉斬 || SkillConstants.is究極突刺(attack.skill)) {
            player.reduceMove();
        }
        int totDamage = 0;
        ...
        //pvp monster bombs
                if (attack.skill != 2301002 && player.getMapId() == 910000000) {//heal
            ...
            if (monster != null) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (!GameConstants.isElseSkill(attack.skill)) {
                        ...
                        if (!GameConstants.isAran(player.getJob())) {
                            atk = SkillConstants.getMaxDamage(player, attack.skill);
                            ...
                            if (ban) {
                                FileoutputUtil.logToFile("logs/Hack/Ban/傷害異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地圖: " + player.getMapId() + " 技能代碼: " + attack.skill + " 最高傷害: " + atk + " 本次傷害 :" + eachd + " 預計傷害: " + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : "") + (getBuffMessage(player).isEmpty() ? "" : " BUFF: " + getBuffMessage(player)));
                        ...
                        if (GameConstants.isAran(player.getJob())) {
                            if (player.getLevel() > 10) {
                                if (eachd > atk && eachd > maxDamagePerHit * 2*30) {
                                    if (eachd != monster.getMobMaxHp()) {
                                        //FileoutputUtil.logToFile("logs/Hack/傷害異常_狂狼.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 技能代碼: " + attack.skill + " 怪物: " + monster.getId() + " 最高傷害: " + atk + " 本次傷害 :" + eachd + " 預計傷害: " + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : "") + (getBuffMessage(player).isEmpty() ? "" : " BUFF: " + getBuffMessage(player)));
                                    }
                                }
                            }
                        ...
                        } else if (eachd > atk && eachd > maxDamagePerHit * 2*3000) {
                    ...
                    if (fixeddmg != -1) {
                        ...
                        if (Tempest) { // Monster buffed with Tempest
                                ...
                                if (eachd > maxDamagePerHit * 2) {
                                    if (eachd > maxDamagePerHit * 2.0D && maxDamagePerHit != 1) {
                                        if (ServerConfig.LOG_DAMAGE) {
                                            FileoutputUtil.logToFile("Logs/hack/傷害計算/傷害計算修正.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 職業: " + player.getJob() + " 怪物:" + monster.getId() + " 技能: " + attack.skill + " 封包傷害 :" + eachd + " 預計傷害 :" + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : "") + (getBuffMessage(player).isEmpty() ? "" : " BUFF: " + getBuffMessage(player)), false, false);
                                            if (player.getLevel() <= 10) {
                                                FileoutputUtil.logToFile("logs/Hack/傷害異常_新手.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 技能代碼: " + attack.skill + " 怪物: " + monster.getId() + " 本次傷害 :" + eachd + " 預計傷害: " + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : "") + (getBuffMessage(player).isEmpty() ? "" : " BUFF: " + getBuffMessage(player)));
                                            }
                                        }
                            ...
                            if (eachd > maxDamagePerHit * 2.0D && maxDamagePerHit != 1) {
                                ...
                                if (ServerConfig.LOG_DAMAGE) {
                                    FileoutputUtil.logToFile("Logs/hack/傷害計算/傷害計算修正.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 職業: " + player.getJob() + " 怪物:" + monster.getId() + " 技能: " + attack.skill + " 封包傷害 :" + eachd + " 預計傷害 :" + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : "") + (getBuffMessage(player).isEmpty() ? "" : " BUFF: " + getBuffMessage(player)));
                                }
                                //player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[傷害: ").append(eachd).append(", 預計傷害: ").append((int) maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 技能: ").append(attack.skill).append("]").toString());
                            }
                        }
                ...
                if (range > SkillRange && !player.inBossMap() && !player.checkUsingPortal()) { // 815^2 <-- the most ranged attack in the game is Flame Wheel at 815 range
                ...
                // 風影漫步
                final MapleStatEffect wind_walk = player.getStatForBuff(MapleBuffStat.WIND_WALK);
                if (wind_walk != null) {
                ...
                if (totDamageToOneMonster > 0) {
                            ...
                            for (int i : skills) {
                                ...
                                if (player.getSkillLevel(skill) > 0) {
                                    ...
                                    if (venomEffect.makeChanceResult()) {
                                        monster.applyStatus(player, new MonsterStatusEffect(MonsterStatus.POISON, 1, i, null, false), true, venomEffect.getDuration(), monster.getStats().isBoss(), venomEffect, totDamage);
                                    }
                                    break;
                                }
                            }
                    ...
                    if (effect != null && (effect.getMonsterStati().size() > 0) && effect.makeChanceResult()) {
                        for (Map.Entry z : effect.getMonsterStati().entrySet()) {
                            MonsterStatusEffect status = new MonsterStatusEffect((MonsterStatus) z.getKey(), (Integer) z.getValue(), theSkill.getId(), null, false);
                            monster.applyStatus(player, status, effect.isPoison(), effect.getDuration(), monster.getStats().isBoss(), effect);
                        }
                    }
                }
            }
    ...
    public static final void applyAttackMagic(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, final MapleStatEffect effect) {
        ...
        if (attack.targets > CheckCount) {
            if (player.hasGmLevel(1)) {
                ...
                //FileoutputUtil.logToFile("logs/Hack/打怪數量異常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 地圖: " + player.getMapId() + "技能代碼: " + attack.skill + " 技能等級: " + player.getSkillLevel(effect.getSourceId()) + " 封包怪物量 : " + attack.targets + " 服務端怪物量 :" + effect.getMobCount());
                //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM 密語系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "攻擊怪物數量異常。 " + "封包怪物量 " + attack.targets + " 服務端怪物量 " + effect.getMobCount() + " 技能ID " + attack.skill));
                return;
            }
        }
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (!GameConstants.isElseSkill(attack.skill)) {
                        if (GameConstants.Novice_Skill(attack.skill)) {//新手技能
                            ...
                            if (eachd > fixdam) {
                                FileoutputUtil.logToFile("logs/Hack/Ban/傷害異常.txt", "\r\n " + FileoutputUtil.CurrentReadable_TimeGMT() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地圖: " + player.getMapId() + " 技能代碼: " + attack.skill + " 技能等級 " + lv + " 最高傷害: " + fixdam + " 本次傷害 :" + eachd + " 預計傷害: " + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : ""));
                                World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("[封鎖系統] " + player.getName() + " 因為使用不法程式而被管理員永久停權。"));
                                World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM 密語系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "傷害異常。 " + "最高傷害 " + fixdam + " 本次傷害 " + eachd + " 技能ID " + attack.skill + " 技能等級 " + lv));
                                player.ban(player.getName() + "傷害異常", true, true, false);
                                player.getClient().getSession().close();
                                return;
                            }
...
//				    System.out.println("EXCEED!!! Client damage : " + eachd + " Server : " + MaxDamagePerHit);
                                    ...
                                    if (eachd >= 1000000000) {
                                        if (ServerConfig.LOG_DAMAGE) {
                                            FileoutputUtil.logToFile("Logs/hack/傷害計算/魔法傷害計算修正.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 職業: " + player.getJob() + " 怪物:" + monster.getId() + " 封包傷害 :" + eachd + " 預計傷害 :" + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : ""), false, false);
                                        }
                                        //player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_MAGIC_2, new StringBuilder().append("[傷害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                        return;
                                    }
                            ...
                            eachd = (int) (maxDamagePerHit);
                            if (eachd > MaxDamagePerHit * 999999 && maxDamagePerHit != 1) {
                                if (ServerConfig.LOG_DAMAGE) {
                                    FileoutputUtil.logToFile("Logs/hack/傷害計算/魔法傷害計算修正.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 職業: " + player.getJob() + " 怪物:" + monster.getId() + " 封包傷害 :" + eachd + " 預計傷害 :" + (int) maxDamagePerHit + (monster.getStats().isBoss() ? " 是否為BOSS: " + monster.getStats().isBoss() : ""), false, false);
                                }
                            }
...
//      }
        ...
        if (chr.getAttackDebugMessage()) {
            chr.dropMessage("[傷害計算]屬性傷害：" + (int) Math.ceil(elemMaxDamagePerMob) + " BOSS傷害：" + (int) Math.ceil(((monster.getStats().isBoss()) ? chr.getStat().bossdam_r : chr.getStat().dam_r) - 100) + "%");
        }
        if (elemMaxDamagePerMob > 19999999) {
            elemMaxDamagePerMob = 19999999;
...
//	}
        ...
        if (player.getAttackDebugMessage()) {
        ...
        if (elementalMaxDamagePerMonster > 19999999) {
            if (!defined) {
                elementalMaxDamagePerMonster = 19999999;
            }
...
//        lea.skip(4); //0
    ...
    private static void CheckWZHack(MapleCharacter player, MapleStatEffect effect, int atk, AttackInfo attack) {
        int CheckCount = effect == null ? (attack.skill == 0 ? 16 : 1) : effect.getMobCount();
        int mpneed = effect == null ? 0 : effect.getMpCon();
        int mp = player.getStat().getMp();
        final boolean unlimitmp = player.isSkillWorking(2121004) || player.isSkillWorking(2221004) || player.isSkillWorking(2321004);
        // MP量檢測
        if (mpneed > 0 && mp < mpneed && !unlimitmp && attack.skill != 2301002 && attack.skill != 21101003 && attack.skill != 2321008 && attack.skill != 2221007 && attack.skill != 2121007) {
            if (player.hasGmLevel(1)) {
                player.dropMessage(6, "MP量異常,技能代碼: " + attack.skill + " 需要MP量 : " + mpneed + "  目前MP量 :" + mp);
            } else {
                //player.getCheatTracker().registerOffense(CheatingOffense.無MP使用技能, " 玩家: " + player.getName() + "(" + player.getLevel() + ") 地圖: " + player.getMapId() + "技能代碼: " + attack.skill + " 技能等級: " + player.getSkillLevel(attack.skill) + " 需要MP量 : " + mpneed + "  目前MP量 :" + mp);
                return;
            }
        } else if (mpneed > 0 && mp < mpneed && (attack.skill == 21101003 || attack.skill == 2301002 || attack.skill == 2321008 || attack.skill == 2221007 || attack.skill == 2121007)) {
            return;
        }

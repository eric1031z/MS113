...
public class InventoryHandler {
    ...
    public static final void UseItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit()) || chr.getMapId() == 610030600 || chr.getMapId() == 105100300) { //cwk quick hack
            if (MapleItemInformationProvider.getInstance().getItemEffect(toUse.getItemId()).applyTo(chr)) {
                chr.addCooldown(toUse.getItemId(),System.currentTimeMillis(),MapleItemInformationProvider.getInstance().getItemEffect(toUse.getItemId()).getDuration()); 
                if(itemId != 2000005){
    ...
    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c, final MapleCharacter chr, final int vegas) {
        ...
        } else if (scrolled.getLevel() > oldLevel || scrolled.getEnhance() > oldEnhance || scrolled.getState() > oldState ) {
        ...
        if (scrollSuccess == Equip.ScrollResult.CURSE) {
            /*if(cursestop!=null){
                 chr.dropMessage("您已免除詛咒之苦");
                 MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE,cursestop.getPosition(), (short) 1, false, false);
            }
            else {*/
...
//        c.getPlayer().updateTick(slea.readInt());
        ...
        switch (itemId) {
            ...
            case 5050000: { // 能力值
                ...
                switch (apto) { // AP to
                    case STR: // str
                        if (playerst.getStr() >= 1999) {
                            used = false;
                        }
                        break;
                        
                    case DEX: // dex
                        if (playerst.getDex() >= 1999) {
                            used = false;
                        }
                        break;
                    case INT: // int
                        if (playerst.getInt() >= 1999) {
                            used = false;
                        }
                        break;
                    case LUK: // luk
                        if (playerst.getLuk() >= 1999) {
                            used = false;
                        }
                        break;
                    case MAXHP: // hp
            ...
            case 5071000: { // Megaphone
                ...
                if (c.getPlayer().isPlayer()) {
                    c.getChannelServer().broadcastSmega(MaplePacketCreator.getMegaphone(sb.toString()));
            ...
            case 5077000: { // 3 line Megaphone
                ...
                World.Broadcast.broadcastSmega(MaplePacketCreator.getTripleSmega(messages, ear, c.getChannel()));
            ...
            case 5072000: { // 高效能喇叭
                ...
                if (c.getPlayer().isPlayer()) {
                    World.Broadcast.broadcastSmega(MaplePacketCreator.getSuperMegaphone(sb.toString(), ear, c.getChannel()));
                } else if (c.getPlayer().isGM()) {
                    World.Broadcast.broadcastSmega(MaplePacketCreator.getSuperMegaphone(sb.toString(), ear, c.getChannel()));
                }
                if (ServerConfig.LOG_MEGA) {
            ...
            case 5073000: { // Heart Megaphone
                ...
                if (c.getPlayer().isPlayer()) {
                    World.Broadcast.broadcastSmega(MaplePacketCreator.getHeartSmega(msg, ear, c.getChannel()));
                } else if (c.getPlayer().isGM()) {
                    World.Broadcast.broadcastSmega(MaplePacketCreator.getHeartSmega(msg, ear, c.getChannel()));
                }
                if (ServerConfig.LOG_MEGA) {
            ...
            case 5074000: { // 骷髏喇叭
                ...
                if (c.getPlayer().isPlayer()) {
                    World.Broadcast.broadcastSmega(MaplePacketCreator.getSkullSmega(msg, ear, c.getChannel()));
                } else if (c.getPlayer().isGM()) {
                    World.Broadcast.broadcastSmega(MaplePacketCreator.getSkullSmega(msg, ear, c.getChannel()));
                }
                if (ServerConfig.LOG_MEGA) {
            ...
            case 5075005: {
                ...
                World.Broadcast.broadcastSmega(MaplePacketCreator.getSuperMegaphone(c.getPlayer().getName() + " : " + message, ear, c.getChannel()));
                break;
            }
    ...
    public static final void PlayerPickup(final LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (mapitem.getMeso() > 0) {
                ...
                /*if(c.getChannelServer().getPlayerStorage().getCharacterById(mapitem.getOwner()).isGM()){
                     World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語] 玩家 " + chr.getName() + " 撿起由GM : " + c.getChannelServer().getPlayerStorage().getCharacterById(mapitem.getOwner()).getName() + " 所丟棄的物品 : " + MapleItemInformationProvider.getInstance().getName(mapitem.getItem().getItemId()) + " 共 " + mapitem.getItem().getQuantity() + " 個" ));
                     FileoutputUtil.logToFile("logs/data/玩家撿GM.txt", "\r\n" + FileoutputUtil.NowTime() + " " + chr.getName() + " 撿起由GM : " + c.getChannelServer().getPlayerStorage().getCharacterById(mapitem.getOwner()).getName() + " 所丟棄的物品 : " + MapleItemInformationProvider.getInstance().getName(mapitem.getItem().getItemId()) + " 共 " + mapitem.getItem().getQuantity() + " 個" );
                }//檢查gm丟棄物品*/

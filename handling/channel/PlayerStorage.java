...
public class PlayerStorage {
    ...
    public PlayerStorage(int channel) {
        ...
        PingTimer.getInstance().schedule(new PersistingTask(), 45 * 60 * 1000);
    }
    ...
    public final Collection<CharacterTransfer> getAllPendingCharacters() {
        readLock.lock();
        try {
            return PendingCharacter.values();
        } finally {
            readLock.unlock();
        }
    }
    ...
    public final void registerPlayer(final MapleCharacter chr) {
        if (System.getProperty("StartBySwing") != null) {
            WvsCenter.getInstance().updateCharTable(true, chr);
        }
        writeLock.lock();
        try {
    ...
    public final void deregisterPlayer(final MapleCharacter chr) {
        if (System.getProperty("StartBySwing") != null) {
            WvsCenter.getInstance().updateCharTable(false, chr);
        }
        writeLock.lock();
        try {
...
//            }
    ...
    public final void deregisterPlayer(final int idz, final String namez) {
        if (System.getProperty("StartBySwing") != null) {
            WvsCenter.getInstance().updateCharTable(false, idToChar.get(idz));
        }
        writeLock.lock();
        try {
            idToChar.remove(idz);
    ...
    public final void broadcastPacket(final byte[] data) {
        ...
        try {
            ...
            while (itr.hasNext()) {
                boolean canSee = true;
                if(data[2] == (byte) 19 && itr.next().getSmega() == false){ //普轉
                    canSee = false;
                }
    ...
    public final void broadcastGMPacket(final byte[] data, boolean 吸怪) {
        ...
        try {
            ...
            while (itr.hasNext()) {
                chr = itr.next();
                if (chr.getClient().isLoggedIn() && chr.isGM() && chr.getmsg_MobVac()) {
                    chr.getClient().sendPacket(data);
                }
            }
        } finally {

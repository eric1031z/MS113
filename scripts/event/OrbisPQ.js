...
function setup(eim, leaderid) {
	...
	em.setProperty("stage7", "1"); //papa spawned
	em.setProperty("stage8", "0"); // 防止最終召喚不出來黑色種子
...
function monsterValue(eim, mobId) {
	if (mobId == 9300049 && em.getProperty("stage7").equals("2")) { //dark nep
	...
	} else if (em.getProperty("stage8").equals("8")) {
		eim.broadcastPlayerMsg(6, "遠古精靈出現了！");
		var mob = em.getMonster(9300039);
		eim.registerMonster(mob);
		em.setProperty("stage7", "0");

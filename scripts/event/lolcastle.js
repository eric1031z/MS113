...
function announce() {
    ...
    if (i == 0) i = 5;
    em.getChannelServer().broadcastPacket(tools.MaplePacketCreator.broadcastMessage(6, "[Event] Field of Judgement will open in " + i + " minutes"));
    i--;
}
...
function mesoDistribution() {
	...
	if (eim.getPlayerCount() > 0) {
	    ...
	    map.broadcastMessage(tools.MaplePacketCreator.broadcastMessage(6, "[Event] " + winner.getName() + " wins " + meso + " meso"));
...
function start() {
	...
	tools.MaplePacketCreator.broadcastMessage(6, "[Event] Field of Judgement is now open"));
    var iter = em.getInstances().iterator();
    while (iter.hasNext()) {
	var eim = iter.next();
	if (eim.getPlayerCount() > 0) {
...
function playerDead(eim, player) {
    ...
    player.getClient().getSession().write(tools.MaplePacketCreator.broadcastMessage(1, "You lost with " + eim.getKillCount(player) + " points."));
}
...
function allMonstersDead(eim) {
	...
	winner.getClient().getSession().write(tools.MaplePacketCreator.broadcastMessage(1, "You win with " + eim.getKillCount(winner) + " points. You will be warped out in 2 minutes."));

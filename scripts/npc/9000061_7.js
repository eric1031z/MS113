...
var rawnItems = [
	//點數裝備
	[1112127, 4000216, 30, 0,1],
	[1122017, 4000216, 30, 0,1],
	[1113021, 4000216, 30, 0,1]
		...
		if (mapId > 0) {
			if (status == 0) {
				for (var i = 0; i < rawnItems.length; i++) {
					msg += "\r\n#L" + i + "##b用" + rawnItems[i][2] + "個#v"+ rawnItems[i][1] + "#兌換"+rawnItems[i][4]+"個#v"+rawnItems[i][0]+"#(15天)";
					//msg += "# " + rawnItems[i][2] + "個";
				}
				cm.sendSimple("#b[QQGM卡兌換專區]#k\r\n如果有蒐集到GM卡片的話可以找我兌換獎勵唷!\r\n你想要兌換哪一種GM卡片的獎勵呢?\r\n"+ msg);
			} else if (status == 1) {
				sel = selection;
				...
				if (cm.canHold(gainItem, amount)) {
					if (cm.haveItem(needitem, quatity) && cm.getPlayer().getMeso() >= meso) {
						...
						cm.gainMeso(-meso);
						cm.gainItem(gainItem, amount,15);
						cm.sendNext("恭喜兌換完成!");
						status = -1;
					} else {

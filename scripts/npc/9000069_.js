    ...
    if (status == 0) {
        ...
        if (!cm.haveItem(2022034, 100)) {
			...
			if (id <= 3) {
				...
				cm.worldMessage(6 ,"[端午節活動] 恭喜:" + cm.getChar().getName() + " 抽中了 "+MapleItemInformationProvider.getInstance().getName(get)+"!!");
			} else if (id <= 15) {
			...
			cm.gainItem(get, 1);
            cm.sendNext("恭喜您抽到#v" + get + "##r#t" + get + "#!");
			cm.dispose();
        }
    }
}
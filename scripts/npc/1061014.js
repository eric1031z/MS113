...
function action(mode, type, selection) {
	...
	case -1:
		...
		default:
			...
			if (lvl >= 101 || lvl <= 49) {
				cm.sendNext("等級不足就給我滾!");
				cm.safeDispose();
				return;
			} else {
				cm.sendNext("目前模式為 #i3994116# 如果你想加入這個模式請按下一步  條件是 等級 50 ~ 等級 100 / 遠征隊人數 1 ~ 30 個");
				break;
			}
		}
		break;
	...
	case 1:
		if (mode == 1) {
			...
			if (lvl >= 50 && lvl <= 100) {
				...
				cm.sendNext("由於您的等級尚未到達條件，無法申請遠征隊隊長。");
			}
		} else {

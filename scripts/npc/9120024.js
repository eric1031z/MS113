...
[4000209, 0.25],[4000210, 0.20],[4000211, 0.15],
[4000212, 0.05],[4000213, 0.10],[4000214, 0.25]
];
var Gems = [
//Ore, Gem
...
function SendGameInstructions() {
    cm.sendNext(Header() +
        "#e遊戲規則:#n\r\n" +
        "1. 藉由點選3個或以上的礦石/寶石來破壞他們獲得分數.\r\n" +
        "2. 每破壞一個礦石都會獲得5分,若破壞3comno以上的礦石,每個礦石會多給予5%分數.\r\n" +
        "3. 礦石有機會會變成寶石,而每個寶石直接點選可獲得20分,以combo消除則為30分.\r\n" +
        "4. 特殊物品將隨機出現,藉由直接點選他們可獲得特殊效果:\r\n" +
        "            #v5021013# - 直接銷出周圍8顆礦石/寶石(特殊物品不會被消除).\r\n" +
        "            #v4031051# - 把周圍8顆礦石轉換成寶石.\r\n" +
        "            #v4031311# - 增加2秒的遊戲時間(只會出現在競速模式)\r\n" +
        "            #v3990021##v3990023# - 直接消除水平方向所有礦石/寶石(紅),把水平方向礦石全部變成寶石(綠)\r\n" +
        "            #v3991008##v3991034# - 直接消除垂直方向所有礦石/寶石(紅),把水平方向礦石全部變成寶石(綠)\r\n" +
        "            #v3990020# - 把垂直跟水平方向礦石/寶石全部消除\r\n" +
        "                         其中消除的礦石價值10分,寶石20分,特殊物品和實體獎勵將不會被消除.\r\n" +
        "5. 實體獎勵將會隨機出現,直接點選她們即可獲得:\r\n" +
        "            #v4000209##v4000210##v4000211##v4000212##v4000213##v4000214#\r\n" +
        "\r\n祝您在咕咕雞遊戲世界遊玩愉快 ~"
        );
}
...
function SendDifficultySelectionMenu() {
    ...
    for (var i = 0;i < Difficulties.length;i++) {
        SendStr += "#L"+i+"#"+Difficulties[i][0]+" - "+Difficulties[i][1]+" 寶石種類 #l\r\n";
    }
    cm.sendSimple(SendStr);
}
...
function GetGameSettingsDescriptionString() {
    ...
    + " - 版面大小: " + BoardSizes[BoardSize][0] + "("+ BoardSizes[BoardSize][1]+"x" + BoardSizes[BoardSize][2]
...
function action(mode, type, selection) {
    if (mode == -1 || mode == 0)
        ...
        if (status == 0) {
            ...
            SendStr += "遊戲進行時間: " + GetGameTime((new Date()).getTime()) + "\r\n";

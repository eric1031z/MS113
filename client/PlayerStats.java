...
public class PlayerStats implements Serializable {
    ...
    public transient boolean equippedWelcomeBackRing, equippedFairy, hasMeso, hasItem, hasVac,hasClone, hasPartyBonus, Berserk = false, isRecalc = false, shareRing, shareRing2, droptree; //730
    public transient int equipmentBonusExp, expMod, dropMod, cashMod, levelBonus;
    public transient double expBuff, dropBuff, mesoBuff, cashBuff, realExpBuff, realDropBuff, realMesoBuff, realCashBuff;
    ...
    public final static short maxStr = 1999;
...
//            lock.unlock();
        ...
        for (IItem item : chra.getInventory(MapleInventoryType.EQUIPPED)) {
            ...
            if (equip.getPosition() == -11) {
                if (GameConstants.isMagicWeapon(equip.getItemId())) {
                    ...
                    element_fire = eqstat.getOrDefault("incRMAI", 100);
                    element_light = eqstat.getOrDefault("incRMAL", 100);
                    element_psn = eqstat.getOrDefault("incRMAS", 100);
                    def = eqstat.getOrDefault("elemDefault", 100);
                }
            }
            ...
            watk += equip.getWatk();
            wdef += equip.getWdef();
            mdef += equip.getMdef(); //pvp
            speed += equip.getSpeed();
            jump += equip.getJump();
            switch (equip.getItemId()) {
                case 1112427: // 冒險家的殘忍之戒 cruel, gives crit + OHKO
                    added_sharpeye_rate += 5;
                    added_sharpeye_dmg += 20;
                    break;
                case 1112428: // 冒險家的霸王之戒 critical, gives crit + OHKO
                    added_sharpeye_rate += 10;
                    added_sharpeye_dmg += 10;
                    break;
                case 1112429: // 冒險家的魔法之戒 magical, gives crit + STUN
                    added_sharpeye_rate += 5;
                    added_sharpeye_dmg += 20;
                    break;
                ...
                case 1812001: // 寵物撿寶術
                    hasItem = true;
                    break;
                /* case 1114400: // 提升衝捲成功率
                    shareRing = true;
                    break;
                 case 1114401: // 提升衝捲成功率
                    shareRing2 = true;
                    break;    
                 case 1372049:
                     droptree = true;
        ...
        if (buff != null) {
            double rate = buff.doubleValue();
            double addrate = buff.doubleValue();
            switch (chra.getBuffSource(MapleBuffStat.DROP_RATE)) {
                case 2022462:// 卡珊德拉的獎勵4
                    addrate = 50.0;
                    rate = 150.0;
                    break;
                ...
                case 2382052:// 食人花卡
                case 2383003:// 黑食人花卡
                    if (!MapConstants.is雲彩公園(chr.get().getMapId())) {
                        rate = 100.0;
                        addrate = 0;
                    }
                    break;
                case 2382001:// 兔子鼓手卡
                case 2383004:// 泥人領導者卡
                    if (!MapConstants.is愛奧斯塔(chr.get().getMapId())) {
                        rate = 100.0;
                        addrate = 0;
                    }
                    break;
                case 2382028:// 發條楓葉鼠卡
                    if (!MapConstants.is赫爾奧斯塔(chr.get().getMapId())) {
                        rate = 100.0;
                        addrate = 0;
                    }
                    break;
                case 2382040:// 天使猴卡
                    if (!MapConstants.is猴子森林(chr.get().getMapId())) {
                        rate = 100.0;
                        addrate = 0;
                    }
                    break;
                case 2383006:// 罈壺卡
                case 2383010:// 蔘罈壺卡
                    if (!MapConstants.is靈藥幻境(chr.get().getMapId())) {
                        rate = 100.0;
                        addrate = 0;
                    }
                    break;
                case 2388000:// 紅寶王卡
                    if (!MapConstants.is維多利亞港地域(chr.get().getMapId())) {
                        rate = 100.0;
                        addrate = 0;
                    }
                default:
        ...
        if (buff != null) {
            double rate = buff.doubleValue();
            double addrate = 0d;
            switch (chra.getBuffSource(MapleBuffStat.MESO_RATE)) {
                case 2022459:// 卡珊德拉的獎勵1
                    rate = 130.0;
                    addrate = 30.0;
                    break;
                case 2022460:// 卡珊德拉的獎勵2
                    rate = 150.0;
                    addrate = 50.0;
                    break;
                case 2022461:// 卡珊德拉的獎勵3
                    rate = 200.0;
                    addrate = 100.0;
                    break;
                case 2382016:// 黑色發條鼠卡
                ...
                for (int i = eq.getEquipLevel() - lvlz; i > 0; i--) {
                    ...
                    if (GameConstants.getStatFromWeapon(eq.getItemId()) == null && GameConstants.getMaxLevel(eq.getItemId()) < (lvlz + i) && Math.random() < 0.1 && eq.getIncSkill() <= 0 && ii.getEquipSkills(eq.getItemId()) != null) {
                        for (int zzz : ii.getEquipSkills(eq.getItemId())) {
                            final ISkill skil = SkillFactory.getSkill(zzz);
...
//                    }
            ...
            chr.forceReAddItem_Equip(eq.copy(), MapleInventoryType.EQUIPPED);
        }
        if (changed) {
            chr.equipChanged();
    ...
    public boolean checkEquipDurabilitys(final MapleCharacter chr, int gain) {
        ...
        for (Equip eqq : all) {
            if (eqq.getDurability() == 0) { //> 0 went to negative
                ...
                chr.forceReAddItem_Equip(eqq.copy(), MapleInventoryType.EQUIPPED);
            }
        }
        return true;
    }
    ...
    private void CalcPassive_SharpEye(final MapleCharacter player, final int added_sharpeye_rate, final int added_sharpeye_dmg) {
        switch (player.getJob()) { // Apply passive Critical bonus
            ...
            case 322: { // Bowman
                ...
                if (critlevel > 0) {
                    this.passive_sharpeye_percent = (short) (critSkill.getEffect(critlevel).getDamage() - 100 + added_sharpeye_dmg);
                    this.passive_sharpeye_rate = (byte) (critSkill.getEffect(critlevel).getProb()*5 + added_sharpeye_rate  );
                    return;
                }
                break;
            }
    ...
    public final float calculateMaxBaseDamage(final int matk, final int watk) {
        ...
        if (watk == 0) {
            ...
            maxbasedamage = ((weapon.getMaxDamageMultiplier() * mainstat) + secondarystat) * (magican ? matk : watk) / 100;
        }
        return maxbasedamage;
    }

    public final float getShouldHealHP() {
        int shouldHealHP = 10;
        Skill bx;
        int bof;
        MapleStatEffect eff;
        ...
        if (chr.get().getChair() != 0) {
            shouldHealHP += 150;
        }
        return shouldHealHP;
    }

    public final float getShouldHealMP() {
        int shouldHealMP = 3;
        Skill bx;
        int bof;
        MapleStatEffect eff;
    ...
    public final void relocHeal() {
        ...
        if (chra.getChair() != 0) { // Is sitting on a chair.
            Pair<Integer, Integer> heal = MapleItemInformationProvider.getInstance().getChairRecovery(chra.getChair());
            if (heal != null) {
                shouldHealHP += heal.left;
                shouldHealMP += heal.right; // MP is different here, if chair data MP = 0, heal + 1.5
            } else {
    ...
    public int getMaxDis(MapleCharacter player) {
        ...
        if (weapon_item != null) {
            ...
            if (weapon == MapleWeaponType.槍 || weapon == MapleWeaponType.矛) {
                maxDis = 106;
            }
            if (weapon == MapleWeaponType.短劍 || weapon == MapleWeaponType.單手劍 || weapon == MapleWeaponType.單手斧 || weapon == MapleWeaponType.單手棍 || weapon == MapleWeaponType.指虎) {
                maxDis = 63;
            }
            if (weapon == MapleWeaponType.雙手劍 || weapon == MapleWeaponType.雙手斧 || weapon == MapleWeaponType.雙手棍) {
                maxDis = 73;
            }
            if (weapon == MapleWeaponType.短杖 || weapon == MapleWeaponType.長杖) {
                maxDis = 51;
            }
            if (weapon == MapleWeaponType.拳套) {
            ...
            if (weapon == MapleWeaponType.弓 || weapon == MapleWeaponType.弩) {
    ...
    public int getRandomage(MapleCharacter player) {
            int maxdamage = (int)player.getStat().getCurrentMaxBaseDamage();
            int mindamage = player.getStat().calculateMinBaseDamage(player);
            return player.getStat().rand(mindamage, maxdamage);
    }//pvp
    
   public byte[] makeHPBarPacket(MapleCharacter player) {
        return MobPacket.showBossHP(9400711, player.getHp(), player.getMaxHp());
   }//pvp
    ...
    public int calculateMinBaseDamage(MapleCharacter player) {
        ...
        if (atk == 0) {
            ...
            if (weapon_item != null) {
                MapleWeaponType weapon = MapleItemInformationProvider.getInstance().getWeaponType(weapon_item.getItemId());
                //mastery start
                if (MapleJob.is劍士(player.getJob())) {
                ...
                //end mastery
                if (weapon == MapleWeaponType.弩) {
                    minbasedamage = (int) (localdex * 0.9 * 3.6 * crossbow + localstr) / 100 * (atk + 15);
                }
                if (weapon == MapleWeaponType.火槍) {
                    minbasedamage = (int) (localdex * 0.9 * 3.6 * crossbow + localstr) / 100 * (atk + 15);
                }
                 if (weapon == MapleWeaponType.指虎) {
                    minbasedamage = (int) (localstr * 0.9 * 3.6 * crossbow + localstr) / 100 * (atk + 15);
                } 
                if (weapon == MapleWeaponType.弓) {
                    minbasedamage = (int) (localdex * 0.9 * 3.4 * bow + localstr) / 100 * (atk + 15);
                }
                if (MapleJob.is盜賊(player.getJob()) && (weapon == MapleWeaponType.短劍)) {

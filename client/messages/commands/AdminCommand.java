...
public class AdminCommand {
    ...
    public static class VIP extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            int vip = 0;
            vip = Integer.parseInt(splitted[2]);;
          
            if (victim != null && player.allowedToTarget(victim) && vip <= 20 && vip>=0) {
                victim.setVip(vip);
                c.getPlayer().dropMessage(6, "[VIP] 您已將"+victim.getName()+"的VIP等級調整為"+vip ); 
            } else if(vip>20||vip<0) {
                c.getPlayer().dropMessage(6, "[VIP] 您的輸入有誤"); 
            } else if(victim==null||! player.allowedToTarget(victim)){
                c.getPlayer().dropMessage(6, "[VIP] 角色不存在");
            }
            return true;
        }
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("VIP <角色名稱> <VIP等級> ...  - 修改玩家vip").toString();
        }
    }
    ...
    public static class GainPoint extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("gainpoint <數量> <玩家> - 取得Point").toString();
        }
    }
    ...
    public static class Item extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("item <道具ID> - 取得道具").toString();
        }
    }
    ...
    public static class ItemVac extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("ItemVac - 全圖吸物開關").toString();
        }
    }
            ...
            if (em != null) {
                if (splitted.length < 2) {
                    ...
                    if (MapleEventType.getByString(s) != null) {
                        ...
                        em.scheduleRandomEventInChannel(StringTool.parseInt(s));
                    }
                } else {
                    em.scheduleSystemEventInChannel(splitted[1], StringTool.parseInt(splitted[2]));
                }
            }
            return true;
        }
    ...
    public static class Monitor extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("monitor <玩家> - 記錄玩家資訊").toString();
        }
    }
    ...
    public static class KillAll extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("killall [range] [mapid] - 殺掉所有怪物").toString();
...
//                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.spawnNPC(npc, true));
    ...
    public static class Drop extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            if (GameConstants.isPet(itemId)) {
                ...
                if(ii.isCash(itemId) && ii.itemExists(itemId)){
                    int uniqueid = MapleInventoryIdentifier.getInstance();
                    toDrop.setUniqueId(uniqueid);
                }
    ...
    public static class 給點數 extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("給點數 點數/楓點 玩家名稱 數量").toString();
        }
    }
    ...
    public static class WM extends CommandExecute {
        
        @Override
        public boolean execute(MapleClient c, String splitted[]){
            
           final int cs = ChannelServer.getChannelCount();
            final int monsterid = MapleWorldMonster.getRandomWM();
            final int rcs = (int)(Math.random()*(cs-1)+1);
            final int l = GameConstants.townMap.length;
            final int mapid = GameConstants.townMap[(int)(Math.random()*l)];
            if(monsterid > 0){
                 MapleMonster mon = MapleLifeFactory.getMonster(monsterid);
                 final OverrideMonsterStats os = new OverrideMonsterStats( Long.parseLong(String.valueOf(MapleWorldMonster.getWM("hp", monsterid)) + "0000000"),MapleWorldMonster.getWM("mp", monsterid),mon.getStats().getExp(),false);
                 mon.setHp((long)(MapleWorldMonster.getWM("hp", monsterid)*10000000));
                 mon.setOverrideStats(os);
            
                 Point p = ChannelServer.getInstance(rcs).getMapFactory().getMap(mapid).getPortal(0).getPosition();
                 
                 ChannelServer.getInstance(rcs).getMapFactory().getMap(mapid).spawnMonsterOnGroundBelow(mon, p);
                 MapleWorldMonster.setWMTime(monsterid, new Timestamp(System.currentTimeMillis()));
                 MapleWorldMonster.setWM("currentmap",monsterid,mapid);
                 MapleWorldMonster.clearWM(monsterid);
                 World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice("【世界野王】" + mon.getStats().getName() + " 已出現於《" +  ChannelServer.getInstance(rcs).getMapFactory().getMap(mapid).getMapName() + "》－　頻道" + rcs));
                 
            }else{
                 World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice("【世界野王】當前無野王資料，請待GM新增!"));
            }
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("WM - 野王").toString();
        }
    }
    
    public static class 登記 extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String splitted[]) {
            ...
            victim = MapleCharacter.getCharacterByName(name);
            if(victim!=null){
                 victim.setPrizeLog(splitted[2]);
                 c.getPlayer().dropMessage(5,"已將 :" + name + " 玩家加入"+ splitted[2] +"領取名單");
            }else{
                c.getPlayer().dropMessage(5,"請確認您輸入的角色是否存在");
            }
            return true;
        }
        ...
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("登記 <角色名稱> <活動LOG>  - 名單登記").toString();
        }
    }
    
    public static class 註冊 extends CommandExecute {
        
        @Override
        public boolean execute(MapleClient c, String splitted[]){
            String login = splitted[1];
            String pwd = splitted[2];
            if(AutoRegister.getAccountExists(login)){
                c.getPlayer().dropMessage("您輸入的帳號 :" + login + " 已存在");
                return true;
            }
            
            if(login.length() > 13){
                c.getPlayer().dropMessage("您輸入的帳號 :" + login + " 太長");
                return true;
            }
            
            if(login.contains(" ")){
                c.getPlayer().dropMessage("您輸入的帳號含有空白符號");
                return true;
            }
            
            AutoRegister.createAccount(login, pwd, c.getSession().remoteAddress().toString(), c.getPlayer().getName() + " GM註冊",3000);
            c.getPlayer().dropMessage("GM您好, 已為您註冊 帳號 " + login  + " 密碼 : " + pwd + " 之帳號") ;
            return true;
        }
        
        @Override
        public String getMessage() {
            return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("註冊 <帳號> <密碼> - 註冊帳號").toString();
        }
    }
    
    public static class 改密 extends CommandExecute {
       @Override
        public boolean execute(MapleClient c, String splitted[]){
            String name = splitted[1];
            String pwd = splitted[2];
            ...
            try {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        c.getPlayer().dropMessage("您輸入的角色名稱無法找到對應的帳號");
                        return true;
                    }
                }
                PreparedStatement pq = con.prepareStatement("UPDATE accounts SET password = ?,salt = ? WHERE id = ?");
                final String newSalt = LoginCrypto.makeSalt();
                pq.setString(1, LoginCrypto.makeSaltedSha512Hash(pwd, newSalt));
                pq.setString(2, newSalt);
                pq.setInt(3, acid);
                pq.execute();
                pq.close();
                
            }catch(Exception e){
                c.getPlayer().dropMessage("您輸入有誤" + e);
                return true;
            }
            
            c.getPlayer().dropMessage("GM您好, 已為您將角色 :" + name + " 玩家的密碼修正為 :" + pwd);
            return true;
                
    }
    @Override
        public String getMessage() {
           return new StringBuilder().append(ServerConstants.PlayerGMRank.超級管理員.getCommandPrefix()).append("改密 <角色ID> <新密碼> - 改玩家密碼").toString();
        }
    }

...
public class BuddyList implements Serializable {
     ...
     * 預設的好友群組
     */
    public static final String DEFAULT_GROUP = "尚未設定群組";
    ...
    public void loadFromDb(int characterId) throws SQLException {
        ...
        PreparedStatement ps = con.prepareStatement("SELECT b.id, b.buddyid, b.pending, c.name as buddyname, b.groupname FROM buddies as b, characters as c WHERE c.id = b.buddyid AND b.characterid = ?");
        ...
        while (rs.next()) {
            ...
            if (rs.getInt("pending") == 1) {
                ...
                put(new BuddyEntry(rs.getLong("id"), buddyname, buddyid, rs.getString("groupname"), -1, true));
            }
        }
        rs.close();
        ps.close();
     ...
     */
    public void addBuddyRequest(MapleClient client, int buddyId, String buddyName, int buddyChannel) {

        this.put(new BuddyEntry(buddyName, buddyId, BuddyList.DEFAULT_GROUP, buddyChannel, false));

        if (pendingReqs.isEmpty()) {

            client.sendPacket(MaplePacketCreator.requestBuddylistAdd(buddyId, buddyName));

        } else {

            BuddyEntry newPair = new BuddyEntry(buddyName, buddyId, BuddyList.DEFAULT_GROUP, -1, false);

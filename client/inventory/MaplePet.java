...
public class MaplePet implements Serializable {
    ...
    public final void saveToDb() {
        ...
        try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE pets SET name = ?, level = ?, closeness = ?, fullness = ?, seconds = ?, flags = ?, excluded = ?  WHERE petid = ?")) {
...
//    }
    ...
    public int[] getExcludedArrays() {
        return excluded.clone();
    }
    ...
    public void setExcluded(List<Integer> excludedList) {
        if (excludedList.size() > this.excluded.length) {
            return;
        }
        int size = this.excluded.length - excludedList.size();
        for (int i = 0 ; i < size ; i++) {
            //System.out.println(i);
            excludedList.add(0);
        }
        setExcluded(excludedList.stream().mapToInt(Integer::intValue).toArray());
    }

    public void setExcluded(int[] excluded) {
        if (excluded.length != this.excluded.length || Arrays.equals(excluded, this.excluded)) {
            return;
        }
        this.excluded = excluded.clone();
        this.changed = true;
    }

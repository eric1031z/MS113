...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        if (ServerConfig.LOG_TRADE) {
            try {
                String IP = chr.get().getClient().getSession().remoteAddress().toString().split(":")[0];
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", IP + " 玩家: " + chr.get().getName() + " 獲得了道具: " + Items + "\r\n");
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", IP + " 玩家: " + chr.get().getName() + " 獲得了楓幣: " + (mesos > 0 ? mesos - GameConstants.getTaxAmount(mesos) : 0) + "\r\n");
            } catch (Exception ex) {
            }
        }
    }
    ...
    public final void cancel(final MapleClient c, final int unsuccessful) {
        ...
        if (ServerConfig.LOG_TRADE) {
            try {
                String IP = chr.get().getClient().getSession().remoteAddress().toString().split(":")[0];
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "IP: " + IP + " 玩家: " + chr.get().getName() + " 拿回了道具: " + Items + "\r\n");
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "IP: " + IP + " 玩家: " + chr.get().getName() + " 拿回了楓幣: " + (mesos > 0 ? mesos : 0) + "\r\n");
            } catch (Exception ex) {
    ...
    public final static void completeTrade(final MapleCharacter c) {
        ...
        if (c.canTrade() && partner.getChr().canTrade()) {
            ...
            if (partner.isLocked()) { // Both locked
                ...
                if (ServerConfig.LOG_TRADE) {
                    try {
                        FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "---------------------------------" + local.getChr().getName() + "和" + partner.getChr().getName() + "交易開始---------------------------------\r\n");
                        FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "時間: " + FileoutputUtil.NowTime() + " 是否成功交易: " + (lz == 0 && lz2 == 0) + " 地圖: " + local.getChr().getMap().getMapName() + "\r\n");
                    } catch (Exception ex) {
                ...
                if (ServerConfig.LOG_TRADE) {
                    try {
                        FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "---------------------------------" + local.getChr().getName() + "和" + partner.getChr().getName() + "交易結束---------------------------------\r\n");
                    } catch (Exception ex) {
        ...
        if (ServerConfig.LOG_TRADE) {
            try {
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "---------------------------------" + Localtrade.getChr().getName() + "和" + Localtrade.getPartner().getChr().getName() + "交易開始---------------------------------\r\n");
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "時間: " + FileoutputUtil.NowTime() + " 是否成功交易: 否 地圖: " + Localtrade.getChr().getMap().getMapName() + "\r\n");
            } catch (Exception ex) {
        ...
        if (ServerConfig.LOG_TRADE) {
            try {
                FileoutputUtil.logToFile("logs/data/交易紀錄.txt", "---------------------------------" + Localtrade.getChr().getName() + "和" + partner.getChr().getName() + "交易結束---------------------------------\r\n");
            } catch (Exception ex) {

...
public class MapleStatEffect implements Serializable {
    ...
    private double hpR, mpR, mmp_temp, mhp_temp;
    private int duration, sourceid, moveTo, x, y, z, itemCon, itemConNo, bulletConsume, moneyCon, cooldown, morphId = 0, expinc, exp, fatigue, fixdamage;
...
//    private List<Pair<Integer, Integer>> randomMorph;
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final byte level) {
        ...
        ret.mhp_temp = MapleDataTool.getInt("mhp_temp", source, 0);
        ret.mp = (short) MapleDataTool.getInt("mp", source, 0);
        ret.mpR = MapleDataTool.getInt("mpR", source, 0) / 100.0;
        ret.mmp_temp = MapleDataTool.getInt("mmp_temp", source, 0);
        ...
        if (ret.overTime && ret.getSummonMovementType() == null && !ret.isEnergyCharge()) {
            ...
            addBuffStatPairToListIfNotZero(statups, MapleBuffStat.MAXHP, (int) (ret.mhpR + ret.mhp_temp));
            addBuffStatPairToListIfNotZero(statups, MapleBuffStat.MAXMP, (int) (ret.mmpR + ret.mmp_temp));
        ...
        if (skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 9001004: // 終極隱藏
                    ...
                    statups.add(new Pair<>(MapleBuffStat.DARKSIGHT, 1));
                    break;
                case 13101006://風影漫步
                ...
                case 2300000:// 魔力吸收
                    ...
                    if (!mob.getStats().isBoss()) {
                        ...
                        if (absorbMp > 0 && mob.canAbsorbMP()) {
                            ...
                            if (applyto.getBuffedValue(MapleBuffStat.MORPH) == null) {
...
//            applyto.getClient().sendPacket(MaplePacketCreator.showSpecialEffect(19));
            ...
            for (final MapleMapObject affectedmo : affecteds) {
                ...
                if (affected != applyfrom && (isGmBuff() || applyfrom.getParty().equals(affected.getParty()))) {
                    if ((isResurrection() && !affected.isAlive()) || (!isResurrection() && affected.isAlive())) {
                        ...
                        if (affected.getBuffedValue(MapleBuffStat.MORPH) == null) {
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity) {
        ...
        if (infinity) {
            applyto.getClient().sendPacket(MaplePacketCreator.giveEnergyCharge(0, duration / 1000));
            applyto.registerEffect(this, starttime, null, applyto.getId());
        } else {
            applyto.cancelEffect(this, true, -1);
            applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveEnergyCharge(applyto.getId(), 10000, duration / 1000), false);
    ...
    private void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            case 打手.偽裝術:
            case 暗影神偷.煙幕彈:
                if (MapConstants.isCakeMap(applyto.getMapId())) {
        ...
        if ((applyfrom.isPlayer() && applyto.isPlayer()) || applyto.isGM()) {
            ...
            switch (sourceid) {
                ...
                default:
                    if (isMorph() || isPirateMorph()) {
                        ...
                        int mountid2 = parseMountInfo(applyto, sourceid);
                        int mountid = parseMountInfo_Pure(applyto, sourceid);
                        if (mountid != 0 && mountid2 != 0) {
                            ...
                            if(sourceid == 1013){
                                mountid = applyto.getCurrentMount(applyto.getId());
                                mountid2 = applyto.getCurrentMount(applyto.getId());
                            }
                            System.out.println(mountid2);
    ...
    public static final int parseMountInfo(final MapleCharacter player, final int skillid) {
        switch (skillid) {
            ...
            case 傳說.怪物騎乘:
                ...
                //MapleInventoryManipulator.equip(player.getClient(), (short), (short) -110);
                //player.getInventory(MapleInventoryType.EQUIPPED).getItem((byte)(-118));

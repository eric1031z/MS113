...
public final class MapleMap {
    ...
    private int modee;
    private boolean respawnTask = false;
    private int respawnOwner = 0;
    ...
    public final Point calcPointBelow(final Point initial) {
        return calcFhAndPointBelow(initial).getRight();
    }

    public final Pair<Integer, Point> calcFhAndPointBelow(final Point initial) {
        ...
        return new Pair(fh.getId(), new Point(initial.x, dropY));
    }
...
//                chr.dropMessage(" 道具: " + de.itemId + " 機率: " + de.chance * chServerrate * chr.getDropMod() * (chr.getStat().realDropBuff - 100.0 <= 0 ? 100 : chr.getStat().realDropBuff - 100) / 100.0 * (showdown / 100.0) + " 最大/小掉落量: " + de.Maximum + "/" + de.Minimum);
//            }
//        }
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            double masterincard = chr.getMonsterBook().isMasterInCards(mob.getId()) ? 1.5 : 1;
            double lastDrop = chr.getStat().realDropBuff - 100.0 <= 0 ? 100 : chr.getStat().realDropBuff - 100;
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * lastDrop / 100.0 * (showdown / 100.0) * masterincard)) {
                ...
                if(chr.getItemFilter().contains(de.itemId)){
                    continue;
                }
        ...
        if (ServerConstants.MobDropMPoint && Randomizer.nextInt(99) < ServerConstants.MobDropMPointRate ) { //可以開關


            HashMap<String, Integer> pointsOfDay = new HashMap<>();
            HashMap<String, Integer> cache = PointsGained.putIfAbsent(chr.getAccountID(), pointsOfDay);
            if (cache != null)
                pointsOfDay = cache;
            ...
            if (count < ServerConstants.MobDropMPointLimit) {
                ...
                chr.dropMessage ("[打怪掉點] 您已從怪物身上獲得 『　" + cash + "　』點，當日獲得點數 "  + (count  + cash)  + "點/"  +  ServerConstants.MobDropMPointLimit  + "");
                chr.modifyCSPoints(2, cash, false);
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {
                    ...
                    if (mc.isAlive()) {
                        ...
                        switch (monster.getId()) {
                            ...
                            case 8820001:
                                ...
                                if (mc.getBuffedValue(MapleBuffStat.MORPH) == null) {
        ...
        if (mobid == 8810018 && mapid == 240060200 && !chr.isGM()) { // Horntail
            ...
            FilePrinter.print(FilePrinter.HorntailLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
            if (speedRunStart > 0) {
            ...
            FilePrinter.print(FilePrinter.PapLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
            if (speedRunStart > 0) {
                type = SpeedRunType.Papulatus;
            }
        } else if (mobid == 9400266 && mapid == 802000111) {
            FilePrinter.print(FilePrinter.TokyoLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
            if (speedRunStart > 0) {
            ...
            FilePrinter.print(FilePrinter.MalaysiaBOSSLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
            if (speedRunStart > 0) {
                if (mobid == 9420549) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("以永不熄滅的熱情所帶領的遠征隊打倒皮卡啾啊！" + chr.getName() + "你們是真正的勇者！真正的勝利者！"));
            /*for (MapleCharacter c : getCharactersThreadsafe()) {
            ...
            FilePrinter.print(FilePrinter.PinkbeanLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        } else if (mobid == 8800002 && mapid == 280030000) {
            /*for (MapleCharacter c : getCharactersThreadsafe()) {
             c.finishAchievement(15);
             }*/
            FilePrinter.print(FilePrinter.ZakumLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        ...
        } else if (mobid == 9400405 && mapid == 800040208) {
            FilePrinter.print(FilePrinter.JapanBOSSLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        } else if ((mobid == 9400408 || mobid == 9400409) && mapid == 800040410) {
            FilePrinter.print(FilePrinter.JapanBOSSLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        } else if (mapid == 802000211) {
            switch (mobid) {
                case 9400263:
                    break;
                case 9400264:
            ...
            FilePrinter.print(FilePrinter.MalaysiaBOSSLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        } else if (mapid == 220080001) {
            switch (mobid) {
                case 8220001:
                    FilePrinter.print(FilePrinter.EBPQLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
                    break;
                case 9400549:
                case 9300028:
                case 8500002:
                    FilePrinter.print(FilePrinter.NBPQLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
                    break;
                case 8220004:
                case 8220005:
                case 8220006:
                ...
                case 9420544:
                case 8800002:
                    FilePrinter.print(FilePrinter.HBPQLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
                    break;
                case 8820001:
                case 9400300:
                    FilePrinter.print(FilePrinter.HEPQLog, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
                    break;
            }
        } else if (mobid == 9300215 && mapid == 925023800) {
            if (chr.getParty() == null) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("[武陵道館公告]: " + chr.getName() + "成功挑戰武陵道場最強的武公。"));
            } else if (chr.getParty().getMembers().size() >= 1) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("[武陵道館公告]: " + chr.getName() + "和他的小夥伴們成功挑戰武陵道場最強的武公。"));
            }
            FilePrinter.print(FilePrinter.Dojodare, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        } else if (mobid == 9300214 && mapid == 925023700) {
            FilePrinter.print(FilePrinter.Dojodare, MapDebug_Log(MapleLifeFactory.getMonster(mobid)));
        }
    ...
    private String MapDebug_Log(final MapleMonster monster) {
        ...
        try {
            ...
            for (MapleCharacter mc : characters) {
                ...
                sb.append(" 等級: ");
                sb.append(mc.getLevel());
                sb.append(" 職業: ");
                sb.append(mc.getJob()).append(" ");
            }
        } finally {
            charactersLock.readLock().unlock();
        }
    ...
    public final void destroyReactor(final int oid) {
        ...
        if (reactor.getDelay() > 0) {
            try {
                ...
                System.err.println("反應物系統: Timer[MapTimer] 出現異常: " + ex.getLocalizedMessage());
            }
        }
    ...
    public final void spawnFakeMonster(final MapleMonster monster) {
		...
		c.sendPacket(MaplePacketCreator.spawnFakeMonster(monster, 0));
            }
        }, null);
        }
        updateMonsterController(monster);
    ...
    public final void spawnMist(final MapleMist mist, final int duration, boolean fake) {
        ...
        try {
            ...
            System.err.println("Mist系統: Timer[MapTimer] 出現異常: " + ex.getLocalizedMessage());
        }
    }
...
//	broadcastMessage(MaplePacketCreator.dropItemFromMapObject(mdrop, mob.getPosition(), dropPos, (byte) 0));
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        chr.setPosition(new Point(chr.getPosition().x, chr.getPosition().y - 70));
        ...
        if (!chr.isHidden()) {
            ...
            if (chr.isGM() && speedRunStart > 0) {
                endSpeedRun();
                broadcastMessage(MaplePacketCreator.getErrorNotice("The speed run has ended."));
            }
        } else {
        ...
        World.PartySearch.checkPartySearch(chr);
        ...
        if (!chr.isClone()) {
            ...
            if (MapConstants.isCakeMap(mapid)) {
                chr.cancelMorphs();
            }
        }
        ...
        if ((mapid == 10000 && chr.getJob() == 0) || (mapid == 130030000 && chr.getJob() == 1000) || (mapid == 914000000 && chr.getJob() == 2000) || (mapid == 900010000 && chr.getJob() == 2001)) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("年糕谷歡迎 : " + chr.getName() + " 的加入！"));
            chr.getMap().startExtendedMapEffect("歡迎來加入 " + chr.getClient().getChannelServer().getServerName() + "。\r\n", 5120007);
            //chr.dropMessage(1, "歡迎來加入 " + chr.getClient().getChannelServer().getServerName() + "。\r\n期盼您會有最好的回憶~");
        }
    ...
    public final List<MapleMapObject> getMapObjects(final List<MapleMapObjectType> MapObject_types) {
        return getMapObjectsInRange(new Point(0, 0), Double.MAX_VALUE, MapObject_types);
    }
    ...
    public final MaplePortal getPortalByScriptName(final String scriptName) {
        for (final MaplePortal port : portals.values()) {
            String pScriptName = port.getScriptName();
            if (pScriptName != null && pScriptName.equals(scriptName)) {
                return port;
            }
        }
        return null;
    ...
    public final SpawnPoint addMonsterSpawn(final MapleMonster monster, final int mobTime, final byte carnivalTeam, final String msg) {
        final Pair<Integer, Point> posInfo = calcFhAndPointBelow(monster.getPosition());
        posInfo.right.y -= 1;
        final SpawnPoint sp = new SpawnPoint(monster, posInfo, mobTime, carnivalTeam, msg);
        if (carnivalTeam > -1) {
    ...
    public final void addAreaMonsterSpawn(final MapleMonster monster, Point pos1, Point pos2, Point pos3, final int mobTime, final String msg) {
        final Pair<Integer, Point> posInfo1 = calcFhAndPointBelow(pos1);
        final Pair<Integer, Point> posInfo2 = calcFhAndPointBelow(pos2);
        final Pair<Integer, Point> posInfo3 = calcFhAndPointBelow(pos3);
        pos1 = posInfo1.getRight();
        pos2 = posInfo2.getRight();
        pos3 = posInfo3.getRight();
        if (pos1 != null) {
            pos1.y -= 1;
        }
        if (pos2 != null) {
        ...
        posInfo2.right = pos2;
        posInfo3.right = pos3;
        monsterSpawn.add(new SpawnPointAreaBoss(monster, posInfo1, posInfo2, posInfo3, mobTime, msg));
    }
    ...
    public void respawn(final boolean force) {
        ...
        if (force) { //cpq quick hack
            final int numShouldSpawn = monsterSpawn.size() -  (respawnTask ? 0 : spawnedMonstersOnMap.get());
    ...
    public int getMaxRegularSpawn(){
        return maxRegularSpawn;
    } 
    ...
    public void checkStates(final String chr) {
        ...
        boolean moremap = false;
        if (sqd != null) {
            if (sqd.getType() == MapleSquadType.horntail) {
                moremap = true;
            }
            if (!moremap) {
                ...
                if (sqd.getType() == MapleSquadType.horntail) {
                    MapleMapFactory mapf = ChannelServer.getInstance(channel).getMapFactory();
                    int[] maps = {240060000, 240060100, 240060200};
                    int people = 0;
                    for (int i = 0; i < maps.length; i++) {
                        people += mapf.getMap(maps[i]).getCharactersSize();
                    }
                    ...
                    if (em != null) {
                        ...
                        if (chr.equals("") || ChannelServer.getInstance(channel).getMapleSquad(sqd.getType()).getMembers().isEmpty()) {
                            em.setProperty("state", "0");
                            em.setProperty("leader", "true");
                            cancelSquadSchedule();
                            sqd.clear();
    ...
    /*public boolean damageMonster(MapleCharacter chr, MapleMonster monster, int damage) {
        if (monster.getId() == 8800000) {
            List<MapleMapObject> objects = chr.getMap().getMapObjects(Arrays.asList(MapleMapObjectType.MONSTER));
        ...
        if (monster.isAlive()) {
            ...
            synchronized (monster) {
                ...
                if (damage > 0) {
                    int monsterhp = (int)monster.getHp();
                    monster.damage(chr, damage, true);
                    if (!monster.isAlive()) { // monster just died
                        killMonster(monster, chr, true,false,(byte) -1);
    ...
    public void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops) {
        killMonster(monster, chr, false, false, (byte) -1);
    }
      
    public boolean getRespawnTask(){
        return respawnTask;
    }
    
    public void setRespawnTask(boolean x){
        respawnTask = x;
    }
    
    
    public int getRespawnOwner(){
        return respawnOwner;
    }
    
    public void setRespawnOwner(int x){
        respawnOwner = x;
    }
    ...
    public void mapMessage(final int type, final String msg) {

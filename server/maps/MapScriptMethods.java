...
public class MapScriptMethods {
    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        if (c.getPlayer().hasGmLevel(5)) {
            c.getPlayer().dropMessage("[系統提示]您已經建立與地圖腳本:" + scriptName + "的連接。(FirstUser)");
        }
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            case killing_BonusSetting: { //spawns monsters according to mapid
                ...
                if (c.getPlayer().getCheckedPyramid()) {
                    c.getPlayer().getMap().resetFully();
                    ...
                    c.getPlayer().setCheckedPyramid(false);
                }
                break;
            }
            case PRaid_D_Fenter: {
            ...
            default: {
                System.err.println("未處理的腳本 : " + scriptName + ", 型態 : onFirstUserEnter - 地圖ID " + c.getPlayer().getMapId());
    ...
    public static void startScript_User(MapleClient c, String scriptName) {
        ...
        if (c.getPlayer().hasGmLevel(5)) {
            c.getPlayer().dropMessage("[系統提示]您已經建立與地圖腳本:" + scriptName + "的連接。(User)");
        }
        String data = "";
        switch (onUserEnter.fromString(scriptName)) {
            ...
            case dojang_QcheckSet: {
                if (c.getPlayer().getMapId() == 925020003) {
                    c.getPlayer().getMap().startMapEffect("恭喜您，已完成武陵道場所有的樓層挑戰。", 5120024);
                }
                c.sendPacket(MaplePacketCreator.enableActions());
                break;
            }
            ...
            default: {
                System.err.println("未處理的腳本 : " + scriptName + ", 型態 : onUserEnter - 地圖ID " + c.getPlayer().getMapId());
                FilePrinter.printError("MapScriptMethods.txt", "未處理的腳本 : " + scriptName + ", 型態 : onUserEnter - 地圖ID " + c.getPlayer().getMapId());
                break;
            }
        }

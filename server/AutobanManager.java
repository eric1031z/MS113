        ...
        try {
            ...
            if (this.points.get(acc) >= AUTOBAN_POINTS) { // See if it's sufficient to auto ban
                ...
                if (PiPiConfig.getAutoban()) {
                    ...
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice("[GM密語] " + c.getPlayer().getName() + "因為使用" + sb + "而被管理員永久停權。"));
                    World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice("[封鎖系統] " + c.getPlayer().getName() + "因為使用違法程式練功而被管理員永久停權。"));

...
import sever.wmonster.MapleWorldMonster;
...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private List<Integer> WMPrizeList = new ArrayList<>();
    ...
    public boolean inWMPrizeList(MapleCharacter chr){
        return WMPrizeList.contains(chr.getId());
    }
    ...
    public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime, final int lastSkill) {
        ...
        for (final AttackerEntry aentry : attackers) {
            if (aentry.equals(attacker) || aentry.contains(from)) {
                attacker = aentry;
                replaced = true;
                break;
            }
        ...
        /*if(MapleWorldMonster.getWM("currentmap", this.getId()) == from.getMapId() && MapleWorldMonster.getWM("mobid", this.getId()) != 0 ){
            from.addDamageWMonster(damage); //增加傷害
            if(from.getDamageWMonster() >= (MapleWorldMonster.getWM("hp", this.getId())*MapleWorldMonster.getWM("prizeneed", this.getId())*100000) && !inWMPrizeList(from)){
                from.dropMessage("【世界野王】恭喜您已達到獎勵門檻傷害占比！獎勵已自動發放"); //獎勵
                from.gainItem(MapleWorldMonster.getWM("prize", this.getId()), MapleWorldMonster.getWM("prizecount", this.getId())); //得到物品
                WMPrizeList.add(from.getId());  
            }
        }*/ 
        ...
        if (false) {
            ...
            if (this.getHp() > 0) {
                ...
                if (from.getLevel() >= 120 && from.getQuestStatus(29400) == 1) {
                    if (this.getLevel() >= 120) {
                        from.addMobCount(from.getMobCount() + 1);
                    }
                }
                /*if(MapleWorldMonster.getWM("currentmap", this.getId()) == from.getMapId() && MapleWorldMonster.getWM("mobid", this.getId()) != 0 ){
                    MapleWorldMonster.setWM("currentmap",this.getId(),0);
                    WMPrizeList.clear();
                    List<Pair<Long,Integer>> damageIndex = new ArrayList<Pair<Long,Integer>>(); //list
                    for (final AttackerEntry mattacker : getAttackers()) {
                        for (final AttackingMapleCharacter cattacker : mattacker.getAttackers()) {
                            Pair<Long,Integer> index = new Pair<>((long)0,0);
                            index.left = cattacker.attacker.getDamageWMonster();
                            index.right = cattacker.attacker.getId();
                            damageIndex.add(index); //加入
                            
                            cattacker.attacker.cleanDamageWMonster();
                            cattacker.attacker.setWMPirze(false); //清除資料
                        }
                    }
                    
                   Collections.sort(damageIndex, Comparator.comparing(p -> -p.getLeft()));
                   MapleCharacter.getCharacterById(damageIndex.get(0).right).gainItem(MapleWorldMonster.getWM("bonus",this.getId()),MapleWorldMonster.getWM("bonuscount",this.getId())); //獲得獎勵
                   World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice("【世界野王】" + this.getStats().getName() + "已被擊倒! 最高累積傷害由玩家: " + MapleCharacter.getCharacterNameById(damageIndex.get(0).right) + " 達成!(" + damageIndex.get(0).left + "總傷害) ！！！")); 
                }*/
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final byte pty, final byte classBounsExpPercent, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            /*if(attacker.getParty()!=null){
               for(MaplePartyCharacter p : attacker.getParty().getMembers()){
            ...
            if(pq > 1){
                exp *= 0;
                attacker.dropMessage("組隊經驗處理異常，請將隊伍內重複角色刪除。");
            }*/
                ...
                if (isAlive()) {
                    ...
                    try {
                        if (getController() != null) {
                            getController().getClient().sendPacket(MobPacket.cancelMonsterStatus(MapleMonster.this, mse));
                        }
                    } catch (Exception ex) {
...
//    }
                ...
                if (con != null && con.getDebugMessage()) {
        ...
        if (reflection.size() > 0) {
            ...
            if (con != null && con.getClient() != null) {
            ...
            for (Entry<MonsterStatus, Integer> z : effect.entrySet()) {
                ...
                //if (effectz != null && con != null) {
                //    con.getClient().sendPacket(MobPacket.applyMonsterStatus(this, effectz));
                //}
            }
        }
        try {
            ...
            if (con != null && con.isShowInfo()) {
                ...
                if (con.getDebugMessage()) {
                    con.dropMessage(5, "開始 => 怪物施放狀態: 持續時間[" + duration + "] 開始時間[" + System.currentTimeMillis() + "] 狀態效果:" + bfn);
                }
            }
        } catch (RejectedExecutionException ex) {
            System.err.println("怪物施放狀態系統: Timer[BuffTimer] 出現異常: " + ex.getLocalizedMessage());
        }
    }
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        applyStatus(from, status, poison, duration, checkboss, eff, 0);
    }

    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff, final int damage) {
        if (!isAlive()) {
            return;
        }
        if (from.hasGmLevel(5)) {
            if (status.getStatus() != null && eff != null) {
                from.dropMessage(6, "怪物: " + getId() + " 狀態: " + status.getStatus().name() + " 是否為毒: " + poison + " 持續時間: " + duration + " 技能: " + SkillFactory.getSkillName(eff.getSourceId()));
            }
        }
        ...
        if (poison && getHp() > 1) { // 中毒[POISON]
            boolean isgunner = from.isPirate();
            boolean isBowman = from.isBowman();
            boolean isMagican = from.isMage();
            boolean isThief = from.isThief();
            double Attackstat = (isThief ? from.getStat().getTotalLuk() : isMagican ? from.getStat().getTotalInt() : isBowman || isgunner ? from.getStat().getTotalDex() : from.getStat().getTotalInt()) / 20000.0;
            Double poisonDamage = Math.min(Short.MAX_VALUE, (getMobMaxHp() * ((0.025 + (from.getSkillLevel(eff.getSourceId())) / 1000.0) + Attackstat)));
            int Dmg = poisonDamage.intValue();
            if (Dmg <= 1) {
                Dmg += Randomizer.rand(10, (int) (getMobMaxHp()));
            }
            //  int poisonDamage = (int) Math.min(Short.MAX_VALUE, (long) (getMobMaxHp() / (70.0 - from.getSkillLevel(status.getSkill())) + 0.999));
            //  花花大神ww
            if (from.hasGmLevel(5)) {
                from.dropMessage(6, "怪物: " + getId() + " 狀態: 毒 技能: " + SkillFactory.getSkillName(eff.getSourceId()) + " 傷害: " + Dmg);
            }
            status.setValue(stat, Dmg);
            status.setX(Dmg);
            // 設定中毒持續傷害，如果status.getX()跟這裡傷害不一樣就會有顯示跟實際不同的問題
            status.setPoisonDamage(Dmg, from);
        ...
        final Runnable applyEffectTask = new Runnable() {
            ...
            public final void run() {
                if (status.getPoisonDamage() != 0 && isAlive()) {
                    if (from.getDebugMessage()) {
                        from.dropMessage(6, "執行 => 持續傷害: 傷害[" + status.getPoisonDamage() + "] 執行時間[" + System.currentTimeMillis() + "]");
                    }
        ...
        final Runnable cancelTask = new Runnable() {
            ...
            public final void run() {
                if (isAlive()) {
                    if (getStati().containsKey(stat)) {
                        MapleMonster.this.cancelStatus(stat);
                        if (from != null && from.isStaff() && from.getDebugMessage()) {
        ...
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.VENOMOUS_WEAPON) {
            ...
            try {
                ...
                poisons.add(status);
                applyMonsterStatus(MobPacket.applyMonsterStatus(this, poisons));
            } finally {
                poisonsLock.writeLock().unlock();
            }
        } else {
            stati.put(stat, status);
            applyMonsterStatus(MobPacket.applyMonsterStatus(this, status));
        }
        try {
            BuffTimer.schedule(cancelTask, aniTime);// + (int) (Math.random() * this.getStati().size() * 1500) + this.getStati().size() * 700
        } catch (RejectedExecutionException ex) {
            System.err.println("怪物狀態系統: Timer[BuffTimer] 出現異常: " + ex.getLocalizedMessage());
        }
        if (from != null && from.getDebugMessage()) {
            from.dropMessage(6, "開始 => 給予怪物狀態: 持續時間[" + aniTime + "] 狀態效果[" + status.getStatus().name() + "] 開始時間[" + System.currentTimeMillis() + "]");
        }
        // 持續傷害的Buff
        if (status.getStatus() == MonsterStatus.POISON || status.getStatus() == MonsterStatus.NINJA_AMBUSH) {
            int reTime = 1000;
    ...
    public final void applyMonsterStatus(final byte[] packet) {
        ...
        if (con != null) {
            ...
            con.getClient().sendPacket(packet);
        } else {
    ...
    public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
        if ((status.getStatus() == MonsterStatus.VENOMOUS_WEAPON || status.getStatus() == MonsterStatus.POISON) && poisons.size() <= 0) {
            return;
        }
        if (status.getStatus() != MonsterStatus.VENOMOUS_WEAPON && status.getStatus() != MonsterStatus.POISON && !stati.containsKey(status.getStatus())) {
            return;
        }
        if (weakChr == null) {
            return;
    ...
    private final class SingleAttackerEntry implements AttackerEntry {
        ...
        public void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            if (chr != null && chr.isAlive()) {
                giveExpToCharacter(chr, baseExp, mostDamage, (byte) 0, (byte) 0, (byte) 0, lastSkill);
            }
        }
...
//                                    Class_Bonus_EXP = ServerConstants.Class_Bonus_EXP(pchr.getJob());
                                ...
                                if (pchr.getStat().equippedWelcomeBackRing && Premium_Bonus_EXP < 80 && pchr.getPartyId()!=-1) {

...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn) {
        CashItem csi = CashItemFactory.getInstance().getItemForGift(sn,chr.getCSType());
        if (csi == null) {
            return;
        }
    ...
    public static IItem addbyId_Gachapon(final MapleClient c, final int itemId, short quantity) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (eItem.getUpgradeSlots() > 0) {//如果該裝備卷軸次數>0
                int old_upgradeSlots = eItem.getUpgradeSlots();//讀取裝備原有的卷軸次數
                int rndNum = (int) Math.floor(Math.random() * 3);   // 0~2 公式
                eItem.setUpgradeSlots((byte) (old_upgradeSlots + rndNum));//原有的卷軸次數+上隨機的公式數字 0~2
            }
            final short newSlot = c.getPlayer().getInventory(type).addItem(item);
...
//            c.setMonitored(true);
    ...
    public static boolean checkSpace(final MapleClient c, final int itemid, int quantity, final String owner) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (slotMax > 0 && GameConstants.isSpaceItem(itemid)) {
                numSlotsNeeded = 1;
            } else if (slotMax > 0 && !GameConstants.isRechargable(itemid)) {
...
//            if (OnlyID.getInstance().CheckDataByID(item.getEquipOnlyId())) {
//                OnlyID.getInstance().removeData(item.getEquipOnlyId());
//            }
        }
    }
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null && initialTarget.getItemId() == source.getItemId() && !GameConstants.isRechargable(source.getItemId()) && !type.equals(MapleInventoryType.CASH) && initialTarget.getOwner().equals(source.getOwner())) {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        switch (dst) {
            ...
            case -11:
                ...
                break;
            /*case -105:
             final IItem csbottom = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -106);
             if (csbottom != null && isOverall(source.getItemId())) {
             if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).isFull()) {
             c.sendPacket(MaplePacketCreator.getInventoryFull());
             c.sendPacket(MaplePacketCreator.getShowInventoryFull());
             return;
             }
             unequip(c, (byte) -106, c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNextFreeSlot());
             }
             break;*/
            default:
                break;
        ...
        if (reqlv > c.getPlayer().getLevel() && !c.getPlayer().isGM()) {
            ...
            //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改裝備(" + source.getItemId() + ")封包，穿上裝備時封鎖。 該裝備需求等級: " + reqlv));
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        int reqlv = MapleItemInformationProvider.getInstance().getReqLevel(source.getItemId());
        if (reqlv > c.getPlayer().getLevel() && !c.getPlayer().isGM()) {
            //FileoutputUtil.logToFile("logs/Hack/Ban/修改封包.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家：" + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改裝備(" + source.getItemId() + ")封包，脫除裝備時封鎖。 該裝備需求等級: " + reqlv);
            //World.Broadcast.broadcastMessage(MaplePacketCreator.getItemNotice("[封鎖系統] " + c.getPlayer().getName() + " 因為修改封包而被管理員永久停權。"));
            //World.Broadcast.broadcastGMMessage(MaplePacketCreator.getItemNotice("[GM密語]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改裝備(" + source.getItemId() + ")封包，脫除裝備時封鎖。 該裝備需求等級: " + reqlv));
            //c.getPlayer().ban("修改封包", true, true, false);
            //c.getSession().close();
            return;
        }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if(c.getPlayer().getMap().getId() == 910000000){
            c.getPlayer().dropMessage(1,"此處無法丟棄物品");
            c.sendPacket(MaplePacketCreator.enableActions());
            return false;
        }
        ...
        if (MapleItemInformationProvider.getInstance().isCash(source.getItemId()) && !npcInduced) {
            c.sendPacket(MaplePacketCreator.enableActions());
            return false;
        }
    ...
    public static void removeAllByEquipByInventoryId(MapleClient c, long inventoryitemid) {
        if (c.getPlayer() == null) {
            return;
        }
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
